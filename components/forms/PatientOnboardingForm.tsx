  ...
  const handleNext = async () => {
    ...
    switch (currentStep) {
      case 0: // DemographicContactStep
        const demographic = methods.getValues('demographicAndContactInformation')
        if (!demographic?.womanFullName || !demographic?.age || !demographic?.bloodType || 
            !demographic?.maritalStatus || !demographic?.state || !demographic?.lga || 
            !demographic?.villageAddress || !demographic?.womanPhoneNumber || 
            !demographic?.womanEducationLevel || !demographic?.emergencyContactName || 
            !demographic?.emergencyContactRelationship) {
          isValid = false
          errorMessage = 'Please fill in all required fields in the Demographics section.'
        }
        break
      case 1: // ObstetricHistoryStep
        const obstetric = methods.getValues('obstetricHistory')
        if (!obstetric?.totalPregnancies || !obstetric?.numberOfLiveBirths || 
            !obstetric?.numberOfPreviousDeliveries || !obstetric?.lastMenstrualPeriod || 
            !obstetric?.estimatedDateOfDelivery) {
          isValid = false
          errorMessage = 'Please fill in all required fields in the Obstetric History section.'
        }
        break
      case 2: // MedicalHistoryStep
        const medical = methods.getValues('medicalHistory')
        if (!medical?.hivStatus || !medical?.hepatitisBStatus) {
          isValid = false
          errorMessage = 'Please select HIV Status and Hepatitis B Status.'
        }
        break
      case 3: // CurrentPregnancyStep
        const pregnancy = methods.getValues('currentPregnancyDetails')
        if (!pregnancy?.currentGestationWeeks || pregnancy.currentGestationWeeks <= 0) {
          isValid = false
