...
export default function SplashScreen({ onFinish }: SplashScreenProps) {
  ...
  const handleFinish = () => {
    if (!hasFinished.current && onFinish) {
      ...
      try {
        ...
        console.error('SplashScreen: Error calling onFinish:', error)
      }
    }
  }
  ...
  useEffect(() => {
    ...
    Animated.timing(loadingAnim, {
      ...
      // Always call handleFinish regardless of animation completion status
      handleFinish()
    })
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#3B82F6', // primary-500 from tailwind config
    alignItems: 'center',
